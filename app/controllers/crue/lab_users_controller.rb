# encoding: UTF-8
class Crue::LabUsersController < ApplicationController #ACTUALIZADA
  require 'net/http'
  ...
  def create
    if Usuario.find_by_email(params[:email])
      ...
      redirect_to crue_laboratorio_lab_users_path(params[:laboratorio_id])
      return
    end
    roles = ''
    counter = 1
    ...
    usuario.laboratorio = Laboratorio.find_by_id params[:laboratorio_id]
    usuario.save!
  ...
  def update
    ...
    redirect_to crue_laboratorio_lab_users_path(usuario.laboratorio.id)
  end
  ...
  def export_xls
    usuarios = Laboratorio.find_by_id(params[:laboratorio_id]).usuarios
    p = Axlsx::Package.new
