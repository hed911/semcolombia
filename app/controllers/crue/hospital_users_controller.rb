...
class Crue::HospitalUsersController < ApplicationController #ACTUALIZADA
  ...
  def index
    ...
    @usuarios = Usuario.where institucion: @institucion
  ...
  def create
    if Usuario.find_by_email(params[:email])
      ...
      redirect_to crue_institucion_hospital_users_path(params[:institucion_id])
      return
    end
    roles = ''
    counter = 1
  ...
  def update
    ...
    redirect_to crue_institucion_hospital_users_path(usuario.institucion.id)
  end
  ...
  def export_xls
    usuarios = Institucion.find_by_id(params[:institucion_id]).usuarios
    p = Axlsx::Package.new
