...
class Laboratorio::CasoSaludPublicasController < ApplicationController
  before_action :authenticate_usuario!

  def index
    @laboratorios = Laboratorio.where(municipio: current_municipio).order("nombre ASC")
    ...
    @usuarios = current_usuario.laboratorio.usuarios.order("primer_nombre ASC")
  end
  ...
  def search_actuales
    ...
    render template: "laboratorio/caso_salud_publicas/search", layout: false
  end

  def search_actualesxxx
    hash = {}
    hash[:laboratorio] = current_usuario.laboratorio
    hash[:numero_documento] = params[:numero_documento].strip if params[:numero_documento] && params[:numero_documento] != ""
    hash[:estado] = ESTADO_SP_VIGENTE
    @casos = CasoSaludPublica.where(hash).includes([:muestras, :entidad_prestadora, :usuario]).order("created_at DESC").paginate(page: params[:index].to_i, per_page: params[:size].to_i)
  ...
  def create
    ...
    @caso.laboratorio = current_usuario.laboratorio
  ...
  def search
    where = ""
    where_lines = []
    where_lines << "(caso.municipio_id = #{current_usuario.laboratorio.municipio.id})"
  ...
  def existe
    ...
    render json: { result: !caso.nil?, institucion: !caso.nil? && !caso.institucion.nil? ? caso.institucion.nombre.upcase : "SIN ASIGNAR" }.to_json
  end
