module SisMuestraHelper
  def self.parse_item(item, municipio)
    puts item["ID"]
    fecha_muestra = DateTime.strptime(item["FechaMuestra"], "%Y-%m-%dT%H:%M:%S") + 5.hours
    fecha_resultado = DateTime.strptime(item["FechaResultado"], "%Y-%m-%dT%H:%M:%S") + 5.hours
    ...
    fecha_nacimiento = item["FechaNacimiento"] ? (DateTime.strptime(item["FechaNacimiento"], "%Y-%m-%dT%H:%M:%S") + 5.hours) : nil
    edad = item["Edad"]
    sexo = item["Sexo"]
    ...
    caso = CasoSaludPublica.find_by_tipo_documento_and_numero_documento_and_municipio_id(tipo_documento, numero_documento.strip, municipio.id)
    caso = CasoSaludPublica.find_by_numero_documento_and_municipio_id(numero_documento.strip, municipio.id) if caso.nil?
    if caso.nil?
      caso = CasoSaludPublica.create! especial: true
      ...
      caso.sexo = sexo
      caso.entidad_prestadora = municipio.entidad_prestadoras.where("unaccent(nombre) ILIKE '%#{eapb.downcase.strip}%'").first if !eapb.downcase.strip.empty?
    ...
    muestra = caso.muestras.where(laboratorio: laboratorio, fecha: (fecha_muestra).beginning_of_day..(fecha_muestra).end_of_day).first
    if resultado.downcase.strip != "en procesamiento"
      if fecha_muestra && muestra
        if resultado.downcase.strip == "positivo"
          ...
          muestra.resultado = 3
        elsif resultado.downcase.strip == "sin procesar" || resultado.downcase.strip == "no procesadas por fallas pre-analiticas"
          muestra.resultado = 4
        end
        muestra.save!
        ...
        muestra.sismuestra_tipoprueba = tipo_prueba
        muestra.sismuestra_eapb = eapb
        muestra.sismuestra_fecha_resultado = fecha_resultado
        muestra.estado = 5
        muestra.sismuestra_laboratorio_notifica = laboratorio_notifica
        muestra.save!
      else
        laboratorio = municipio.laboratorios.where("unaccent(nombre) ILIKE '%#{I18n.transliterate(laboratorio_notifica.downcase.strip)}%'").first
        if laboratorio.nil?
        ...
        muestra = Muestra.create! estado: 5, especial: true
        if resultado.downcase.strip == "positivo"
          muestra.resultado = 2
        elsif resultado.downcase.strip == "negativo"
          muestra.resultado = 3
