...
sheet1.rows.each_slice(1000) do |group|
  ...
  group.each do |r|
    ...
    caso = CasoSaludPublica.where(tipo_documento:tipo_documento, numero_documento:.strip, municipio:Municipio.find(22)).first
    if caso.nil?
      ...
      caso.sexo = sexo
      if eapb
        caso.entidad_prestadora = Municipio.find(22).entidad_prestadoras.where("unaccent(nombre) ILIKE '%#{eapb.downcase.strip}%'").first
      end
      if ips_envia
        caso.institucion = Municipio.find(22).institucions.where("unaccent(nombre) ILIKE '%#{ips_envia.downcase.strip}%'").first
      end
      caso.country = Country.find_by_codigo("co")
      caso.departamento = Municipio.find(22).departamento
      caso.municipio = Municipio.find(22)
      if departamento
    ...
    if fecha_muestra && muestra
      ...
      laboratorio = Municipio.find(22).laboratorios.where("unaccent(nombre) ILIKE '%#{I18n.transliterate(laboratorio_notifica.downcase.strip)}%'").first
      if laboratorio.nil?
        laboratorio = Laboratorio.create! nombre: laboratorio_notifica.downcase.strip
        laboratorio.municipio = Municipio.find(22)
        laboratorio.save!
      end
